...
public class Optimizations {
  ...
  // ADD_BEGIN
  public static boolean ENABLE_MATCH_IP_OPTIMIZATION = false;

  public static boolean ENABLE_MATCH_COMMUNITY_OPTIMIZATION = false;

  public static boolean ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION = false;
  // ADD_END
  ...
  private Map<String, TreeSet<Long>> _localPrefAbstraction;
  // ADD_END
  ...
  Optimizations(EncoderSlice encoderSlice) {
    ...
    // ADD_BEGIN
    _dstPrefixes = new HashSet<>();
    _localPrefAbstraction = new HashMap<>();
    // ADD_END
  }

  public static void enableReduction() {
    ENABLE_MATCH_IP_OPTIMIZATION = true;
    ENABLE_MATCH_COMMUNITY_OPTIMIZATION = true;
    ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION = true;
  }

  public static void disableReduction() {
    ENABLE_MATCH_IP_OPTIMIZATION = false;
    ENABLE_MATCH_COMMUNITY_OPTIMIZATION = false;
    ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION = false;
  }
  ...
  void computeOptimizations() {
    ...
    // ADD_BEGIN
    // enable multiple path for DCN dataset (naive method)
    if(!_encoderSlice.getGraph().getRouters().contains("aggregation-0")){
      computeRouterIdNeeded();
    }
    computeDstPrefixes();
    computeLocalPrefAbstraction();
    // ADD_END
  }
  ...
  // ADD_BEGIN
  /*
   * find BGP announced Prefixes that intersect with dst_ips
   */
  private void computeDstPrefixes() {
    if (!Optimizations.ENABLE_MATCH_IP_OPTIMIZATION) {
      return;
    }
    _encoderSlice
            .getGraph()
            ...
            .forEach(
                    (router, conf) -> {
                      PrefixSpace prefixSpace = conf.getDefaultVrf().getBgpProcess().getOriginationSpace();
                      Set<PrefixRange> prefixRanges = prefixSpace.getPrefixRanges();
                      for (PrefixRange p : prefixRanges){
                        IpSpace dstIps = _encoderSlice.getHeaderSpace().getDstIps();
                        if (dstIps instanceof IpWildcardIpSpace){
                          IpWildcard dstIpSpaceWildcard = ((IpWildcardIpSpace) dstIps).getIpWildcard();
                          if(dstIpSpaceWildcard.intersects(IpWildcard.create(p.getPrefix()))){
                            _dstPrefixes.add(p.getPrefix());
                          }
                        }
                      }
                    });
  ...
  /*
   * find LocalPref that occurred in each router
   */
  // todo: only consider set-localPref statement, and  LiteralLong type now.
  public void computeLocalPrefAbstraction(){
    if (!Optimizations.ENABLE_LOCALPREF_ABSTRACTION_OPTIMIZATION) {
      return;
    }
    Boolean[] val = new Boolean[1];
    val[0] = false;
    _encoderSlice
            ...
            .forEach(
                    (router, conf) ->{
                      TreeSet<Long> localPrefValues = new TreeSet<>();
                      localPrefValues.add(100L);
                      conf.getRoutingPolicies()
                              .forEach(
                                      (name, pol) -> {
                                        AstVisitor v = new AstVisitor();
                                        v.visit(
                                                ...
                                                stmt -> {
                                                  if (stmt instanceof SetLocalPreference) {
                                                    if(!(((SetLocalPreference) stmt).getLocalPreference() instanceof LiteralLong)){
                                                      System.out.println("Have not consider this type now:\t" + ((SetLocalPreference) stmt).getLocalPreference().getClass());
                                                    }
                                                    else {
                                                      localPrefValues.add(((LiteralLong) ((SetLocalPreference) stmt).getLocalPreference()).getValue());
                                                    }
                                                  }
                                                },
                                                expr -> {});
                                      });
                      _localPrefAbstraction.put(router,localPrefValues);
                    });
  }

  public Map<String,TreeSet<Long>> getLocalPrefAbstraction(){
    return _localPrefAbstraction;
  }

  public Long getAbstractLocalPrefValue(String router, Long originalLP){
    Long res = null;
    TreeSet<Long> localPrefValues = _localPrefAbstraction.get(router);
    Long index = 1L;
    for (Long e : localPrefValues) {
      if (e == originalLP) {
        res = index;
        break;
      }
