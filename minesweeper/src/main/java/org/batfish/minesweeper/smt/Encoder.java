...
public class Encoder {
  ...
  // ADD_BEGIN
  public static final int TIMEOUT = 3600000 * 100;  // milesecond
  // ADD_END
  ...
  private HeaderLocationQuestion _qOrig;
  ...
  Encoder(Graph graph, HeaderQuestion q) {
    this(null, graph, q, null, null, null, 0, null);
  }

  /**
   * Create an encoder object that will consider all packets in the provided headerspace.
   * tag to get original q
   * 
   * @param graph The network graph
   */
  Encoder(Graph graph, HeaderQuestion q, HeaderLocationQuestion qOrig) {
    this(null, graph, q, null, null, null, 0, qOrig);
  }
  ...
  Encoder(Encoder e, Graph g) {
    this(e, g, e._question, e.getCtx(), e.getSolver(), e.getAllVariables(), e.getId() + 1, null);
  }
  ...
  Encoder(Encoder e, Graph g, HeaderQuestion q) {
    this(e, g, q, e.getCtx(), e.getSolver(), e.getAllVariables(), e.getId() + 1, null);
  }
   ...
   * @param q A header question
   */
  Encoder(Encoder e, Graph g, HeaderQuestion q, HeaderLocationQuestion qOrig) {
    this(e, g, q, e.getCtx(), e.getSolver(), e.getAllVariables(), e.getId() + 1, qOrig);
  }
...
//        _solver = _ctx.mkSolver();
  ...
  public Tuple<VerificationResult, Model> verify() {
    ...
    Global.setParameter("timeout", String.valueOf(TIMEOUT));
    
    if(_qOrig instanceof SmtBenchmarkPlugin.BenchmarkQuestion){
      if(((SmtBenchmarkPlugin.BenchmarkQuestion) _qOrig).getGuided()){
        // System.out.println("!!!!getGuided:");
        Global.setParameter("guided", "true");
        
        String propType = ((SmtBenchmarkPlugin.BenchmarkQuestion) _qOrig).getProptype();
        // prop_type 0:reachability 1:isolation 1:forwarding
        Global.setParameter("prop_type", propType);

        Global.setParameter("topology", Graph.ABSOLUTE_BGP_TOPOLOGY_PATH);

        Global.setParameter("dst_port",_qOrig.getFinalIfaceRegex());

        String networkType = ((SmtBenchmarkPlugin.BenchmarkQuestion) _qOrig).getNetworkType();
        // network_type 0:wan, 1:fattree
        Global.setParameter("network_type", networkType);

        Global.setParameter("dst",_qOrig.getFinalNodeRegex());
      }else{
        // System.out.println("????unGuided:");
        Global.setParameter("guided", "false");
      }
    }

    if(_qOrig instanceof SmtBenchmarkPropertiesQuestionPlugin.SmtBenchMarkPropertiesQuestion){
      if(((SmtBenchmarkPropertiesQuestionPlugin.SmtBenchMarkPropertiesQuestion) _qOrig).getGuided()){
        // System.out.println("!!!!getGuided:");
        Global.setParameter("guided", "true");

        String propType = ((SmtBenchmarkPropertiesQuestionPlugin.SmtBenchMarkPropertiesQuestion) _qOrig).getProptype();
        // prop_type 0:reachability 1:isolation 1:forwarding
        Global.setParameter("prop_type", propType);
        ...
        String networkType = ((SmtBenchmarkPropertiesQuestionPlugin.SmtBenchMarkPropertiesQuestion) _qOrig).getNetworkType();
        // network_type 0:wan, 1:fattree
        Global.setParameter("network_type", networkType);
    ...
    if (_question.getBenchmark()) {
      ...
      Statistics statistics = _solver.getStatistics();

      if(statistics.get("conflicts") == null){
        stats.setConflicts(0);       
      }else{
        stats.setConflicts(statistics.get("conflicts").getUIntValue());
      }

      if(statistics.get("decisions") == null){
        stats.setDecisions(0);       
      }else{
        stats.setDecisions(statistics.get("decisions").getUIntValue());
      }

      // stats.setDecisions(statistics.get("decisions").getUIntValue());
      stats.setMaxZ3Time(statistics.get("time").getDoubleValue());
      stats.setMinZ3Time(statistics.get("time").getDoubleValue());
      stats.setAvgZ3Time(statistics.get("time").getDoubleValue());
      // stats.setMaxZ3TotalTime(time);
      // stats.setMinZ3TotalTime(time);
      // stats.setAvgZ3TotalTime(time);
    }
    ...
    if (status == Status.UNSATISFIABLE) {
      ...
      if (_qOrig instanceof BenchmarkQuestion) {
        VerificationResult res = new VerificationResult(true, null, null, null, null, null, stats);
        return new Tuple<>(res, null);    
      } else {
  ...
  private void addUpLinksConstraints(Set<ArithExpr> vars) {
    for (ArithExpr var : vars) {
      add(mkEq(var, mkInt(0)));
    }
  }

  private void addDownLinksConstraints(Set<ArithExpr> vars) {
    for (ArithExpr var : vars) {
      add(mkEq(var, mkInt(1)));
    }
  }

  VerificationStats addFailedLinkConstraintsByPrune(HeaderLocationQuestion _question, VeriBoost veriboost) {
    int k = _question.getFailures();
    Set<ArithExpr> vars_up = new HashSet<>();
    Set<ArithExpr> vars_down = new HashSet<>();
    Set<ArithExpr> vars_free = new HashSet<>();
    String src_node = _question.getIngressNodeRegex();
    String dst_node = _question.getFinalNodeRegex();
    veriboost.getMinesweeperConstraint(src_node, dst_node);
    HashSet<VeriBoostUtil.SimpleLink> links_up = veriboost.getMinesweeperConstraint(VeriBoostUtil.LinkType.up_link);
    HashSet<VeriBoostUtil.SimpleLink> links_down = veriboost.getMinesweeperConstraint(VeriBoostUtil.LinkType.down_link);
    HashSet<VeriBoostUtil.SimpleLink> links_free = veriboost.getMinesweeperConstraint(VeriBoostUtil.LinkType.free_link);

    VerificationStats status = new VerificationStats();

    int tolerance_space_origin = links_up.size() + links_down.size() + links_free.size();
    int tolerance_space_prune = links_up.size() + links_free.size();
    int tolerance_space_compression = links_free.size();

    status.setPruneTime(veriboost.prune_time);
    status.setCompressionTime(veriboost.compression_time);
    status.setToleranceSpaceOrigin(tolerance_space_origin);
    status.setToleranceSpacePrune(tolerance_space_prune);
    status.setToleranceSpaceCompression(tolerance_space_compression);

    getSymbolicFailures().getFailedInternalLinks().forEach((router, peer, var) -> {
      VeriBoostUtil.SimpleLink link = new VeriBoostUtil.SimpleLink(router, peer);
      if(links_up.contains(link)) {
        vars_up.add(var);
      }
      if(links_down.contains(link)) {
        vars_down.add(var);
      }
      if(links_free.contains(link)) {
        vars_free.add(var);
      }
    });

    getSymbolicFailures().getFailedEdgeLinks().forEach((ge, var) -> vars_up.add(var));
    addFailedConstraints(k, vars_free);
    addUpLinksConstraints(vars_up);
    addDownLinksConstraints(vars_down);

    return status;
  }

  VerificationStats computeEncoding(VeriBoost veriboost, HeaderLocationQuestion question) {
    if (_graph.hasStaticRouteWithDynamicNextHop()) {
      throw new BatfishException(
              "Cannot encode a network that has a static route with a dynamic next hop");
    }
    VerificationStats status = addFailedLinkConstraintsByPrune(question, veriboost);
    addFailedNodeConstraints(_question.getNodeFailures());
    getMainSlice().computeEncoding();
    for (Entry<String, EncoderSlice> entry : _slices.entrySet()) {
      String name = entry.getKey();
