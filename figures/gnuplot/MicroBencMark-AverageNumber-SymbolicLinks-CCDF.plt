...
set output 'results/MicroBencMark-AverageNumber-SymbolicLinks-CCDF.pdf'
set size 1, 0.9

stats 'data/MicroBencMark-AverageNumber-SymbolicLinks-CCDF.dat' using 1 nooutput
N = STATS_records

plot \
  '< sort -n -k1,1 data/MicroBencMark-AverageNumber-SymbolicLinks-CCDF.dat' using ($1*100):(100.*(N-($0+1)+1)/N) with lines ls 3 lw 7 dt 3 title 'Link Compression', \
  '< sort -n -k2,2 data/MicroBencMark-AverageNumber-SymbolicLinks-CCDF.dat' using ($2*100):(100.*(N-($0+1)+1)/N) with lines ls 2 lw 7 dt 2 title 'Link Pruning', \
  '< sort -n -k3,3 data/MicroBencMark-AverageNumber-SymbolicLinks-CCDF.dat' using ($3*100):(100.*(N-($0+1)+1)/N) with lines ls 5 lw 7 dt 1 title 'Both Methods'